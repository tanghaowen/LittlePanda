...
def mvImageToLib(imagePath , fileHash:str , sqlManager ,image_width ,image_heigh,imgLibPath = "img/" , uploader = "" , remark="" ):
    ...
    if image_id != None:
        ...
        update images set path = '%s' where image_id = %d
        """%( imagePath,image_id ))
    ...
    print("---------:获取图片size%0.10f"%(time.time()-starttime))
    starttime=time.time()
    print(imagePath)
    print(hashImagePath)
    ...
    starttime=time.time()
    if uploader == "":
        image_id = sqlManager.addImageToSqlWhithoutCommit(fileHash, imagePath , image_width, image_heigh, file_size, dt , remark=remark)
    else:
        image_id = sqlManager.addImageToSqlWhithoutCommit(fileHash, imagePath,image_width, image_heigh, file_size, dt , uploader=uploader ,remark = remark)
...
def addNewArchFileToImgLibWithProgress( archFilename , sqlManager ,title,uploadedArchPath, status,index , tmpLibPath = "tmp/", imgLibPath = "img/" , uploader = "" , remark = "" , coverPage = 1,lpfs_dll=None):
    same_title_book_id = sqlManager.title2bookId( title)
...
def addNewSingleCoverImageFileToDatabaseWithBookId( imageFilename , sqlManager,book_id ,title, createTime,tmpLibPath = "tmp/", imgLibPath = "img/" , uploader = "" , remark = "" , coverPage = 1 ,lpfs_dll = None):
